...
static void s3c2440_hwcontrol(struct mtd_info *mtd, int cmd, unsigned int ctrl)
{
	...
	if (ctrl & NAND_CTRL_CHANGE) {
		...
		/* wx: if not CLE, then use ALE
		 * why below condition is codeing so this?
		 * because this means if not use CLE, MUST Clear this bit Obvisely, 
		 * or else we will use ALE default.
		 */
		if (!(ctrl & NAND_CLE))
